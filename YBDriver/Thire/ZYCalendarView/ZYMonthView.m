...
- (UILabel *)titleLab {
    if (!_titleLab) {
        _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(0, gap+10,self.frame.size.width, weekH-10)];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor colorWithHexString:@"#f1f3f4" andAlpha:1.0];
        [self commonInit];
    }
    return self;
}
